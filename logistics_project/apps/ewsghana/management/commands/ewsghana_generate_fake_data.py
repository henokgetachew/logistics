...
from logistics_project.apps.ewsghana.loader import init_static_data
...
from logistics_project.apps.ewsghana.migration import check_router
#from logistics_project.apps.ewsghana.models import DeliveryGroups, SupplyPointStatus, SupplyPointStatusTypes, SupplyPointStatusValues
#from logistics_project.apps.ewsghana.utils import supply_points_below
...
class Command(LabelCommand):
    ...
    def handle(self, *args, **options):
        ...
        def populate_facilities(date):
            ...
            for fac in SupplyPoint.objects.all():  #SupplyPoint.objects.filter(type__code='facility'):
        ...
        for date in dates:
            ...
            facs = SupplyPoint.objects.all() #SupplyPoint.objects.filter(type__code='hospital')
            print (facs.count())
	    for fac in facs:
                if random() < .1:
            ...
            for fac in facs:
            #    SupplyPointStatus.objects.create(supply_point=fac,  #No Supply point status for EWS
            #        status_type=SupplyPointStatusTypes.SOH_FACILITY,
            #        status_value=SupplyPointStatusValues.REMINDER_SENT,
            #        status_date=date)
                if random() < submit_chance:
                    report_soh(fac, date)
            #    SupplyPointStatus.objects.create(supply_point=fac,	#No Supply point status for the EWS
